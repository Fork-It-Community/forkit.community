...
<Image src={leVillageByCa} alt={frontmatter.location.name} />
...
<strong>{frontmatter.location.name}</strong> Join us in that awesome
location. Enjoy the Seine river from the heart of Rouen.

<strong>{dayjs(frontmatter.date).format("dddd, MMMM D, YYYY")}</strong> Come
enjoy the last spring days, entertaind with awesome conferences.
